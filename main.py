...
if uploaded_file is not None:
    ...
    data_features_train = [False for _ in range(
    ...
    with col2:
        ...
        if selectbox_label != None:
            ...
            st.dataframe(data.loc[:, features_label].to_frame().style.background_gradient(
                cmap='bone'), use_container_width=True)
    ...
    if selectbox_label != None:
        ...
        feature_select_obj = [
            feature for feature in feature_select if data[feature].dtypes == 'object']
    ...
    # ============================================ Model selection ============================================
        st.markdown(styles.lines_section_separate_style,
        ...
        with col3:
            if button_train_test_split == 'Train test split':
                slider_train_test_split = st.slider(
                    'Training rate', 0.0, 0.99, 0.8)
                st.markdown(
                    f"Train size accounts for {slider_train_test_split * 100}% dataset")
                input_random_state = st.number_input(
                    'Random state', min_value=0)
        ...
        with col1:
            option_model = st.selectbox(
                ...
                ('Logistic Regression', 'Decision Tree Classifier', 'KNeighbors Classifier',
                 'GaussianNB', 'Random Forest Classifier', 'XGBoost Classifier')
            )
            ...
            if st.button("Run"):
                ...
                if button_train_test_split == "Train test split":
                    X_train, X_test, y_train, y_test = train_test_split(
                        X, y, train_size=slider_train_test_split, random_state=input_random_state)
                ...
                ps = precision_score(
                    y_test_max, y_pred_max, average='weighted')
                ...
                st.bar_chart(pd.Series([ps, rs, fs], index=[
                             'Precision score', 'Recall score', 'F1 score']), use_container_width=True)

                with col3:
                    pred_df = pd.DataFrame(
                        {'Actual Value': y_test_max, 'Predicted Value': y_pred_max, 'Same': y_test_max == y_pred_max})
                    # if flag_label_is_obj == True:
                    #     for i in range(0, len(y_pred)):
                    #         pred_df["Actual Value"][i] = mapping_label[i]
                    #         pred_df["Predicted Value"][i] = mapping_label[i]
                    st.dataframe(pred_df, use_container_width=True)
                    count = [pred_df["Actual Value"][i] == pred_df["Predicted Value"][i]
                             for i in range(0, len(pred_df))].count(True)
                    st.markdown(
                        f"""
